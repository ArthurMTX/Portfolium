...
import { formatAssetType, formatCurrency, formatLargeNumber, formatNumber, formatWithSeparators } from '@/shared/lib/formatUtils'
    ...
    <div className="card p-5 sm:p-6 space-y-4">
      ...
      <div className="space-y-4">
        {themeExposure.map((item) => {
          ...
          const percent = Math.max(0, Math.min(100, item.weight * 100))

          return (
            <div key={item.theme} className="space-y-2">
              <div className="flex items-center justify-between gap-3 text-sm">
                <div className="flex min-w-0 items-center gap-2 font-medium text-neutral-800 dark:text-neutral-200">
                  <ThemeIcon size={15} className={themeColor} />
                  <span className="truncate">{item.theme}</span>
                </div>
                <div className="font-semibold text-neutral-900 dark:text-neutral-100">{formatNumber(percent, 1)}%</div>
              </div>
              <div className="h-2 overflow-hidden rounded-full bg-neutral-200 dark:bg-neutral-800">
                <div
                  className="h-full rounded-full"
                  style={{ width: `${percent}%`, backgroundColor: themeHex }}
                />
              </div>
            </div>
          )
