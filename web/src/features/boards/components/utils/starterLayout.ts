...
/** Starter widget layout for a brand-new board, seeded onto a blank board when the user doesn't pick a template. */
export const starterLayouts: { lg: Layout[]; md: Layout[]; sm: Layout[] } = {
  lg: defaultLayout,
  md: tabletLayout,
  sm: mobileLayout,
}
...
/**
 * Re-applies current widget min/max size constraints to a persisted layout.
 * Widget constraints can change between releases (e.g. a widget's minH increases);
 * this keeps old saved boards from rendering widgets below their current minimum size.
 */
export function reconcileLayoutConstraints(layout: Layout[]): Layout[] {
